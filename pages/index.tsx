        ...
        <Layout>
            ...
            <div>
                ...
                <p>Redux 只能接收序列化（serializable）的資料，因為 Redux 需要能夠 serializable 的方式來管理應用程式狀態（state）。這是 Redux 中一個很重要的原則之一。
                    ...
                    序列化的資料是指可以被轉換成固定格式的資料。例如，JSON 就是一種常用的序列化格式，可讓您輕鬆地將 JavaScript 物件轉換為純文字字符串。由於 JSON 是一個基於文本的格式，它不僅易於閱讀和理解，而且還可以被許多編程語言和平台支援。

                    當您嘗試將非序列化的資料存儲到 Redux 中時，會發生錯誤，因為沒有一個好的方法可以將這些資料轉換為純文字字符串或其他易於傳輸且通用的格式。如果您真的需要將非序列化的資料傳遞到 Redux 中，您可以使用額外的庫或插件來處理這些資料。

                    在 Redux 中，最好的做法是確保您的 state 只包含序列化的資料。這意味著您需要確保您的 action 和 reducer 只處理序列化的資料。如果您需要在 state 中存儲非序列化的資料，可以考慮將其轉換為序列化格式後再存儲。</p> */}
